          ...
          <div className="flex justify-between h-16 items-center">
            ...
            <div className="hidden lg:flex items-center space-x-2">
              {NAV_LINKS.map(link => (
                ...
                >
                  {link.label}
                  <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-0.5 bg-gradient-to-r from-purple-600 to-pink-600 transition-all duration-300 ${
                    isActive(link.href) 
                      ? 'w-full' 
              ...
              {NAV_LINKS.map(link => (
                ...
                >
                  <span className={`w-1 h-6 rounded-full mr-3 transition-all duration-200 ${
                    isActive(link.href) ? 'bg-gradient-to-b from-purple-600 to-pink-600' : 'bg-transparent'
                  }`}></span>
                  {link.label}
                </Link>
              ))}
