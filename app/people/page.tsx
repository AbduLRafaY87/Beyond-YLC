...
import { Search, MapPin, Github, Linkedin, Globe, UserPlus, Filter, X, Users, Briefcase, GraduationCap, Sparkles, Award } from 'lucide-react'
...
const ProfileCardSkeleton = () => (
  <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg overflow-hidden border border-white/20 animate-pulse">
...
// Optimized Profile Card with lazy loading
const ProfileCard = ({ profile }: { profile: Profile }) => {
  ...
  return (
    <div className="group bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300 border border-white/20 hover:scale-105">
    ...
    const fetchProfiles = async () => {
      try {
        const { data, error } = await supabase
          .from('profiles')
          .select('id, full_name, avatar_url, bio, location, linkedin, github, website, skills, interests, occupation, organization, ylc_batch, created_at')
          .eq('privacy', 'public') // Only show public profiles
  ...
  const filteredProfiles = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(profile =>
        ...
        profile.occupation?.toLowerCase().includes(search) ||
